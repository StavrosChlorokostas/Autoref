...
@click.command()
@click.option('-i', 'inputfile', type=click.File('rb', lazy=True), help='Input Image')
@click.option('-o', 'outputfile', type=click.File('wb', lazy=True), help='Keypoints output')
@click.option("-feature", "-f", 'feature_name', default='brisk', help="<sift|surf|orb|akaze|brisk>[-flann]")
@click.option("-n-features", "-n", 'n_features', default=0, help="Number of features")
